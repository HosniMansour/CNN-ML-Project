...
def load_pictures(BGR):
    ...
    for k, char in map_characters.items():
        pictures = [k for k in glob.glob('SimpsonDataset/%s/*' % char)]
...
def get_dataset(save=False, load=False, BGR=False):
    ...
    """
    if load:
        h5f = h5py.File('../models/simpson/dataset.h5', 'r')
        X_train = h5f['X_train'][:]
        X_test = h5f['X_test'][:]
        h5f.close()

        h5f = h5py.File('../models/simpson/labels.h5', 'r')
        y_train = h5f['y_train'][:]
        y_test = h5f['y_test'][:]
        h5f.close()
    else:
        ...
        if save:
            h5f = h5py.File('../models/simpson/dataset.h5', 'w')
            h5f.create_dataset('X_train', data=X_train)
            h5f.create_dataset('X_test', data=X_test)
            h5f.close()

            h5f = h5py.File('../models/simpson/labels.h5', 'w')
...
def training(model, X_train, X_test, y_train, y_test, data_augmentation=True):
    ...
    if data_augmentation:
        ...
        datagen.fit(X_train)
        filepath = "../models/simpson/weights_6conv.hdf5"
...
def predict(filename):
    ...
    print(output)
    for i in range(len(map_characters)):
        if (output[0][i] > 0.5):
            print(map_characters[i])


if __name__ == '__main__':
    X_train, X_test, y_train, y_test = get_dataset(save=True, load=True)
    model, opt = create_model_six_conv(X_train.shape[1:])

    # Load The Model !!

    model.load_weights('weights_6conv.hdf5')

    # Train The Model !if no load
    ...
    # open('modeel.json', 'w').write(model_json)

    # preduction and tests
